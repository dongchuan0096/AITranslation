...
class DetSolver(BaseSolver):
    ...
    def test(self):
        ...
        test_stats, coco_evaluator = evaluate(
            ...
            self.test_dataloader, self.evaluator, self.device
        )
        if self.output_dir:
            # 保存测试集评估结果
            dist_utils.save_on_master(coco_evaluator.coco_eval["bbox"].eval, self.output_dir / "test_eval.pth")
            # 记录各项指标到log
            with (self.output_dir / "test_log.txt").open("a") as f:
                import json
