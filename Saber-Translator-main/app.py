...
def mask_image_fields(data):
    """
    递归隐藏常见图片字段内容，避免日志打印图片数据。
    """
    if isinstance(data, dict):
        masked = {}
        for k, v in data.items():
            if k.lower() in ["image", "images", "img", "file", "files"]:
                masked[k] = "[图片数据已隐藏]"
            else:
                masked[k] = mask_image_fields(v)
        return masked
    elif isinstance(data, list):
        return [mask_image_fields(item) for item in data]
    else:
        return data
...
def log_response(response):
    print(f"\n=== 响应内容 {response.status_code} ===")
    print("===============================\n")
    return response

if __name__ == '__main__':
    # 只在主进程打开浏览器，热重载子进程不打开
