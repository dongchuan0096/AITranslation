...
def translate_image():
    ...
    try:
        ...
        # 打印除图像数据外的所有参数
        params_to_log = {k: v for k, v in data.items() if k != 'image'}
        logger.info(f"请求参数（不含图片数据）: {json.dumps(params_to_log, ensure_ascii=False)}")
        ...
        logger.info(f"返回参数 keys: {list(response_data.keys())}")
        return jsonify({
            'code': '0000',
            'msg': '翻译成功',
            'data': response_data
